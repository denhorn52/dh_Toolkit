...
**GUI Builder:** Design GUIs for your dcripts.
...
**dh_Toolkit_themes.lua:** A module providing several predefined themes, and functions for handling loading them. It also adds additional GUI colors.

## **Custom or modified Lokasenna classes:** (in /classes directory)

Some dh_Toolkit classes have minimal differences from the Lokasenna classes; mainly some changed property names and default colors to facilitate theming. Some classes have added properties and enhanced functionality and customization. 

## **Installation:**

The dh_Toolkit_v1 directory (and all its files and subdirectories) can be placed anywhere under the Reaper/Scripts directory. The dh_Toolkit_v1/library directory contains the file "Set dh_Toolkit v1 library path.lua". In Reaper "Action list" browse for and run that file. That will save the dh_Toolkit library path in Reaper Ext State. dh_Toolkit scripts look there to load its core files. This way user scripts can be placed anywhere. Of course, Lokasenna GUI v2 must also be installed (using its installation method). 
