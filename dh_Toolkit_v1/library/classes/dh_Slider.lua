...
---------------------------------------------------------------------
GUI.dh_Slider = GUI.Element:new()

function GUI.dh_Slider:new(name, z, x, y, w, caption, min, max, defaults, inc, dir)

  local Slider = (not x and type(z) == "table") and z or {}

  Slider.name = name
  Slider.type = "dh_Slider"
  ...
  Slider.thk = Slider.thk or 8
  if Slider.thk < 8 then Slider.thk = 8 end

  Slider.dir = Slider.dir or dir or "h"
  
  -- if horizontal w = w,   h = thk
  -- if vertical   w = thk, h = w

  Slider.w, Slider.h = table.unpack(Slider.dir ~= "v"
                        and {Slider.w or w, Slider.thk}
                        or  {Slider.thk, Slider.w or w} )
  
  Slider.caption = Slider.caption or caption
  

  Slider.font_caption = Slider.font_caption or "sans20"
  Slider.font_text = Slider.font_text or "sans16"

--colors-------------------------------------
  Slider.col_bg = Slider.col_bg or "wnd_bg"
  Slider.col_outline = Slider.col_outline or "btn_outline"    
  Slider.col_text = Slider.col_text or "txt"
  Slider.col_track = Slider.col_track or "elm_bg"  
  Slider.col_thumb = Slider.col_thumb or "btn_face"
  Slider.col_fill = Slider.col_fill or "track_fill"  --"elm_fill"
---------------------------------------------
...
function GUI.dh_Slider:init()
    --GUI.Msg("dh_Slider:init self.name is : " .. tostring(self.name))
    ...
    GUI.roundrect(0, 0, w, h, r, 1, 1)
                
    GUI.color(self.col_outline)
    GUI.roundrect(0, 0, w, h, r, 1, 0)
    
    -- Thumb

    local hw, hh = table.unpack(self.dir == "h" and {self.thk, self.thk * 2} or {self.thk * 2, self.thk})

    -- buffer includes space for handle and shadow.
    ...
    gfx.set(r, g, b, 1)
    GUI.roundrect(hw + 2, 2, hw + 2, hh + 2, 2, 1, 1)    
    
    gfx.muladdrect(hw + 2, 2, hw + 2, hh + 2, 1, 1, 1, a, 0, 0, 0, 0 )    

end


function GUI.dh_Slider:ondelete()
...
function GUI.dh_Slider:draw()
    --GUI.Msg("> dh_Slider:draw self.name is : " .. tostring(self.name))
    local x, y, w, h = self.x, self.y, self.w, self.h
    --GUI.Msg("> dh_Slider:draw self.h is : " .. tostring(self.h))
    
    -- Draw track
    
    gfx.blit(self.buffs[1], 1, 0, 0, 0, w + 4, h + 4, x, y)    
    ...
    --x, w = x + 4, w - 8
    x, w = x + (self.thk / 2), w - self.thk
        
    -- Size of the handle
    self.handle_w, self.handle_h = self.thk, self.thk * 2
...
function GUI.dh_Slider:val(newvals)
...
function GUI.dh_Slider:onmousedown()
...
function GUI.dh_Slider:onwheel()
...
function GUI.dh_Slider:ondoubleclick()
...
function GUI.dh_Slider:updatehandlecoords(x, handle_y, inc)
...
function GUI.dh_Slider:drawfill(x, y, h, min, max, inc)
...
function GUI.dh_Slider:setfill()
...
function GUI.dh_Slider:drawsliders()
...
function GUI.dh_Slider:drawslidervalue(x, y, sldr)
    ...
    GUI.text_bg(output, self.col_bg, self.align_values + 256)
    gfx.drawstr(output, self.align_values + 256, gfx.x, gfx.y)

end


function GUI.dh_Slider:drawsliderhandle(hx, hy, hw, hh)
...
function GUI.dh_Slider:drawcaption()
    ...
    GUI.text_bg(self.caption, self.col_bg)
    GUI.shadow(self.caption, self.col_text, "shadow")
    --]]
...
function GUI.dh_Slider:getnearesthandle(val)
...
function GUI.dh_Slider:setcurstep(sldr, step)
...
function GUI.dh_Slider:setcurval(sldr, val)
...
function GUI.dh_Slider:setretval(sldr)
