...
---------------------------------------------------------------------
GUI.dh_Tabs = GUI.Element:new()
function GUI.dh_Tabs:new(name, z, x, y, tab_w, tab_h, opts, pad)
...
function GUI.dh_Tabs:draw()
	...
	for i = #self.optarray, 1, -1 do
		...
		if i ~= state then
			...
			self:draw_tab(tab_x, tab_y, tab_w, tab_h, dir, font, self.col_text, self.col_tab_inactive, self.optarray[i])

		end

	end

	self:draw_tab(x + (state - 1) * x_adj, y, tab_w, tab_h, dir, self.font_tab_active, self.col_text, self.col_tab_active, self.optarray[state])
...
function GUI.dh_Tabs:val(newval)
...
function GUI.dh_Tabs:onmousedown()
...
function GUI.dh_Tabs:onmouseup()
...
function GUI.dh_Tabs:draw_tab(x, y, w, h, dir, font, col_txt, col_bg, lbl)
